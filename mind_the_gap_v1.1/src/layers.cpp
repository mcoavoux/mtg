...
std::default_random_engine Parameter::random(Parameter::SEED);
const int Parameter::SEED = 1;
const double Parameter::ETA = 0.01;
...
void VecParam::update(double lr, double T, double clip, bool clipping, bool gaussian, double gaussian_eta){
    if (gaussian){
        double sigma = pow(gaussian_eta / pow(1.0 + T, 0.55), 0.5);
        std::normal_distribution<double> distribution(0.0, sigma);
        db->array() = db->unaryExpr([&distribution](double x) -> double { return x + distribution(random); });
    }
    if (clipping){
