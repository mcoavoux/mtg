...
void Treebank::write_conll(const string& filename, vector<vector<std::pair<String, String> > > &str_sentences){
    int depfield_id = enc::hodor.get_dep_idx();
    ofstream os(filename);
    for (int i = 0; i < trees_.size(); i++){
        trees_[i].write_conll(os, str_sentences[i], depfield_id);
        os << endl;
    }
    os.close();
}
...
void Treebank::parse_tbk_tree(const vector<String> &sentence){
    shared_ptr<Node> node;
    int idx = 0;
    get_tbk_tree(sentence, 0, sentence.size()-1, idx, node);
    trees_.push_back(Tree(node));
}

bool Treebank::get_tbk_tree(const vector<String> &sentence, int d, int f, int &idx, shared_ptr<Node> &res){
    if (d == f){ // Gérer les -head
        ...
        while (i < f){
            ...
            if (get_tbk_tree(sentence, i, end, idx, node)){
                head = stack.size();
            }
...
int Treebank::find_closing_bracket(int i, const vector<String> &sentence){
    if (! is_xml_markup(sentence[i]))
        return i;
    //cerr << "i = " << i << "   sentence[i] =" << sentence[i] << endl;
    assert(is_xml_beg(sentence[i]));
    String label;
    get_xml_label(sentence[i], label);
    int n = 1;
    while (n != 0){
        i++;
        //cerr << "searching : " << sentence[i] << endl;
    ...
    String close_label;
    get_xml_label(sentence[i], close_label);
#ifdef DEBUG
...
bool Treebank::get_xml_label(const String &s, String &label){ // return true if is head
    int beg = 1;
    if (is_xml_end(s)){
        beg = 2;
    }
...
bool Treebank::is_head(String &s){
