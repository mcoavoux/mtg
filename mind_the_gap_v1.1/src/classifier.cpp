...
//                std::uniform_real_distribution<> dis(0.0, 1.0);
                ...
                //cerr << "Replace by unknown threshold = " << threshold << "  " << enc::hodor.decode_to_str(word_code, enc::TOK) << endl;
                if (rd::random() < threshold){
                    //cerr << "Replace by unknown threshold = " << threshold << endl;
...
void BiRnnFeatureExtractor::assign_tags(vector<shared_ptr<Node>> &buffer){
    for (int i = 0; i < buffer.size(); i++){
        ...
        int argmax;
        output->maxCoeff(&argmax);
//        cerr << "tag = " << enc::hodor.decode_to_str(argmax, enc::TAG) << endl;
//        cerr << enc::hodor.code(enc::hodor.decode(argmax, enc::TAG), enc::CAT) << endl;
//        cerr << enc::hodor.decode_to_str(enc::hodor.code(enc::hodor.decode(argmax, enc::TAG), enc::CAT), enc::CAT) << endl;
...
Rnn::Rnn(int n_classes, const string &outdir) : Classifier(n_classes){
    FeatureExtractor *fe = FeatureExtractor::import_model(outdir);
...
Rnn::Rnn(int n_classes, FeatureExtractor *fe, NeuralNetParameters &params) :
        Classifier(n_classes),
        params_(params){
